...
class DataManager:
    ...
    def _alloc_special_type_on_low_lat_mem(self, site, obj, storage):
        """
        Allocate special types in the low latency memory e.g PETScArrays.
        These types are typically customised versions of ArrayBasic
        that deviate from the standard types
    ...
    def place_definitions(self, iet, globs=None, **kwargs):
        ...
        for i in FindSymbols().visit(iet):
            if i in defines:
                ...
                self._alloc_special_type_on_low_lat_mem(iet, i, storage)
